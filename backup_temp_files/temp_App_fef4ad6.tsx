...
import { Search, Sparkles, User, Calendar, BrainCircuit, Share2, HelpCircle, RefreshCcw, Edit3, CheckCircle2 } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import QuickMBTI from './components/QuickMBTI';

const isValidMBTI = (mbti: string) => /^[EI][SN][TF][JP]$/i.test(mbti);
...
function App() {
  ...
  const [showMBTIPickerFor, setShowMBTIPickerFor] = useState<'user' | 'idol' | null>(null);
  ...
  // Fetch Popular Idols
  const fetchPopular = async (mbti?: string) => {
    try {
      const res = await axios.get(`${API_URL}/idols/popular`, {
        params: { user_mbti: mbti }
      });
  ...
  // 사용자 MBTI가 변경될 때마다 추천 목록 갱신 (개인화)
  React.useEffect(() => {
    if (userMBTI && userMBTI.length === 4) {
      fetchPopular(userMBTI);
    }
  }, [userMBTI]);

  // 수동 모드에서 MBTI 입력 완료 시 에러 메시지 자동 초기화
  React.useEffect(() => {
    if (isManualMode && idolData?.mbti && isValidMBTI(idolData.mbti)) {
      setErrorMessage(null);
    }
  }, [idolData?.mbti, isManualMode]);
  ...
  const executeSearch = async (searchName: string) => {
    ...
    try {
      ...
      // 인물 우선 정렬 (is_person=true인 항목을 앞으로)
      const sortedCands = [...cands].sort((a: any, b: any) => (a.is_person === b.is_person ? 0 : a.is_person ? -1 : 1));

      // 인물 후보가 2개 이상이거나, 후보가 여러 개인데 첫 번째가 인물인 경우 경합을 위해 선택 UI 노출
      if (sortedCands.length >= 2 && sortedCands.filter((c: any) => c.is_person).length >= 2) {
      ...
      // 후보가 1개뿐이거나, 인물 후보가 단 하나여서 명확한 경우 → 바로 검색 진행
      const bestCand = sortedCands.length > 0 ? sortedCands[0] : null;
      const wiki_title = bestCand ? bestCand.title : '';
      const wiki_lang = bestCand ? (bestCand.lang || 'en') : '';
      ...
      <section id="user-profile-section" className="bg-transparent md:bg-slate-800/30 border-none md:border md:border-slate-700/50 rounded-none md:rounded-3xl p-3 md:p-6 mb-8 backdrop-blur-none md:backdrop-blur-sm shadow-none md:shadow-xl">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          ...
          <div className="space-y-2">
            ...
            <div
              onClick={() => setShowMBTIPickerFor('user')}
              className={`w-full bg-slate-900 border-2 rounded-xl px-4 py-3 outline-none transition-all cursor-pointer flex justify-between items-center ${userMBTI.length > 0 && !isValidMBTI(userMBTI)
                ? 'border-red-500 ring-2 ring-red-500/20'
                : userMBTI.length === 4
                  ? 'border-emerald-500 ring-2 ring-emerald-500/20'
                  : 'border-slate-700 hover:border-k-purple'
                }`}
            >
              <span className={userMBTI ? 'text-white font-bold' : 'text-slate-500'}>
                {userMBTI || "e.g. ENFP"}
              </span>
              ...
              >
                {popularIdols.map((idol: any, idx: number) => (
                  <motion.button
                    ...
                    className="flex-shrink-0 w-32 bg-slate-800/50 border border-slate-700/50 rounded-2xl p-3 hover:bg-slate-700/50 hover:border-k-purple/50 transition-all text-center group relative overflow-hidden"
                  >
                    {/* Chemistry Badge */}
                    {idol.chem_score && (
                      <div className="absolute top-2 right-2 z-10">
                        <div className="bg-k-purple/80 text-[8px] font-black px-1.5 py-0.5 rounded-full text-white backdrop-blur-md shadow-lg border border-white/10 flex items-center gap-0.5">
                          {idol.chem_score}%
                        </div>
                      </div>
                    )}
                    ...
                    <div className="w-full aspect-square bg-slate-900 rounded-xl mb-3 flex items-center justify-center overflow-hidden border border-slate-700 group-hover:border-k-purple/30">
                      <img
                        src={`/avatars/${idol.gender === 'male' ? 'male' : 'female'}_${idol.img_id || (idx % 4) + 1}.png`}
                        alt={idol.name_en}
                        className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity"
                        onError={(e: any) => {
                    ...
                    <div className="flex items-center justify-center gap-1">
                      ...
                      {idol.chem_label && (
                        <span className="text-[9px] text-k-pink font-bold">· {idol.chem_label}</span>
                      )}
                    </div>
                  </motion.button>
                ))}
          ...
          >
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <div
                  onClick={() => setShowMBTIPickerFor('idol')}
                  className={`w-full bg-slate-900 border-2 rounded-xl px-4 py-3 outline-none transition-all cursor-pointer flex justify-between items-center ${idolData?.mbti?.length > 0 && !isValidMBTI(idolData.mbti)
                    ? 'border-red-500'
                    : idolData?.mbti?.length === 4
                      ? 'border-emerald-500 ring-2 ring-emerald-500/20'
                      : 'border-slate-700 hover:border-k-blue'
                    }`}
                >
                  <span className={idolData?.mbti ? 'text-white font-bold' : 'text-slate-500'}>
                    {idolData?.mbti || "e.g. ENFP"}
                  </span>
                  <Edit3 className="h-4 w-4 text-slate-500" />
                </div>
              </div>
      ...
      <AnimatePresence>
        {(errorMessage || isManualMode) && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            className={`max-w-2xl mx-auto mb-6 px-6 py-4 rounded-2xl flex items-center gap-4 border shadow-lg transition-all duration-500 ${isManualMode && isValidMBTI(idolData.mbti)
          ...
          >
            {isManualMode && isValidMBTI(idolData.mbti) ? (
            ...
            <div>
              ...
              <p className="text-sm opacity-90 mb-2">
                {isManualMode && isValidMBTI(idolData.mbti)
                  ? "대상자의 MBTI 정보가 정상 입력되었습니다."
                  : errorMessage || (isManualMode ? t('enterManually') : '')}
              </p>
              {isManualMode && !isValidMBTI(idolData.mbti) && (
                <button
              ...
              {isManualMode && (
                <div className="flex flex-wrap items-center gap-4 p-4 bg-slate-800/30 rounded-xl border border-slate-700/30 flex-shrink-0">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <div
                      onClick={() => setShowMBTIPickerFor('idol')}
                      className="bg-slate-900/60 text-sm font-bold text-k-purple outline-none w-24 px-3 py-2 rounded-lg border border-slate-700 hover:border-k-purple transition-all cursor-pointer flex justify-between items-center"
                    >
                      <span className={idolData.mbti ? 'text-k-purple' : 'text-slate-600'}>
                        {idolData.mbti || "MBTI"}
                      </span>
                    </div>
                  </div>
        ...
        {showMBTIPickerFor && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => setShowMBTIPickerFor(null)}
            ...
            <motion.div
              ...
              className="relative w-full max-w-md bg-slate-900 border border-slate-700 p-6 rounded-[2rem] shadow-2xl overflow-hidden"
            >
              <div className="absolute top-0 right-0 w-32 h-32 bg-k-purple/10 rounded-full -translate-y-16 translate-x-16 blur-3xl" />
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-black text-white flex items-center gap-2">
                  <BrainCircuit className="h-5 w-5 text-k-purple" /> Select MBTI
                </h3>
                <button
                  onClick={() => setShowMBTIPickerFor(null)}
                  className="p-2 hover:bg-slate-800 rounded-full transition-colors"
                >
                  <RefreshCcw className="h-5 w-5 text-slate-400 rotate-45" />
                </button>
              </div>
              <div className="grid grid-cols-4 gap-3">
                {MBTI_TYPES.map((type) => (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    onClick={() => {
                      if (showMBTIPickerFor === 'user') setUserMBTI(type);
                      else setIdolData((prev: any) => ({ ...prev, mbti: type }));
                      setShowMBTIPickerFor(null);
                    }}
                    className={`py-3 rounded-xl font-bold text-sm transition-all border-2 ${(showMBTIPickerFor === 'user' ? userMBTI : idolData?.mbti) === type
                      ? 'bg-gradient-to-br from-k-purple/20 to-k-blue/20 border-k-purple text-white shadow-lg'
              ...
              <p className="text-[10px] text-slate-500 mt-6 text-center font-bold tracking-tight opacity-60">
                PROHIBITED MANUAL TYING · SELECTION ONLY
              </p>
            </motion.div>
          </div>
        )}
