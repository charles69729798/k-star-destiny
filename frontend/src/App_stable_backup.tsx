...
const App = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-10 max-w-4xl min-h-screen relative z-10">
          ...
          >
            ...
            <div className="grid md:grid-cols-2 gap-12 items-center relative z-10">
                  ...
                  <div className="flex items-center gap-4 bg-slate-900/80 p-5 rounded-2xl border border-slate-700/50 relative">
                    ...
                    {(!idolData.mbti || idolData.mbti === 'Unknown') && (
                      <button
                        onClick={handleAssistantSearch}
                        disabled={assistantLoading}
                        className="absolute right-4 top-1/2 -translate-y-1/2 px-3 py-1.5 bg-gradient-to-r from-blue-600/20 to-indigo-600/20 hover:from-blue-600/40 hover:to-indigo-600/40 rounded-lg text-xs font-bold text-blue-300 border border-blue-500/30 flex items-center gap-1 transition-all"
                        title="구글 검색으로 자동 수집"
                      >
                        {assistantLoading ? <RefreshCcw className="h-3 w-3 animate-spin" /> : <Search className="h-3 w-3" />}
                        구글 자동 수집
              ...
              <div className="relative group">
                ...
                <div className="relative aspect-square bg-slate-900 rounded-[2rem] flex flex-col items-center justify-center border-2 border-slate-700 p-8 text-center shadow-inner">
                  {!analysisResult ? (
                    <>
      ...
      <footer className="mt-20 text-center text-slate-600 text-xs font-bold tracking-widest uppercase pb-10">
        {t('footer')}
      </footer>
    </div>
  );
