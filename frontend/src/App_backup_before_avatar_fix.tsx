                ...
                {popularIdols.map((idol: any, idx: number) => (
                  ...
                  >
                    <div className="w-full aspect-square bg-slate-900 rounded-xl mb-3 flex items-center justify-center overflow-hidden border border-slate-700 group-hover:border-k-purple/30">
                      <img
                        src={`/avatars/${idol.gender === 'male' ? 'male' : 'female'}_${(idol.name_en.length % 4) + 1}.png`}
                        alt={idol.name_en}
                        className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity"
                        onError={(e: any) => {
            ...
            <div className="flex flex-col md:flex-row items-center gap-4 bg-slate-900/80 rounded-2xl p-4 mb-6 border border-slate-700/50 relative z-10">
                ...
                <div className="min-w-0">
                  {isManualMode ? (
                    <input
                      ...
                      className="bg-transparent border-b-2 border-k-pink text-xl font-black uppercase tracking-tighter outline-none w-full"
                    />
                  ) : (
              ...
              {isManualMode && (
                <div className="flex items-center gap-3 flex-shrink-0">
                  ...
                  <div className="flex items-center gap-2">
                    <BrainCircuit className="h-4 w-4 text-k-purple" />
                    <input
                      ref={mbtiInputRef}
                      type="text"
                      placeholder="MBTI"
                      ...
                      readOnly
                      onClick={() => {
                        setQuizForFriend(true);
                        setShowMBTIQuiz(true);
                      }}
                      className="bg-transparent text-sm font-bold text-k-purple outline-none w-20 border-b border-slate-700 cursor-pointer hover:border-k-purple transition-all"
                    />
                  </div>
                  ...
                  {(!idolData.mbti || idolData.mbti === 'Unknown') && (
                    <p className="text-xs text-amber-300/80 mt-1 ml-1">{t('mbtiHint')}</p>
                  )}
                </div>
              )}
            </div>
